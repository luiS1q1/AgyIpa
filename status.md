...
* **Build-Fix #10 (Vollbildmodus & Seitenverhältnis)**: Ergänzen des `UILaunchScreen`-Eintrags in der `Info.plist`. Dies behebt die dicken schwarzen Ränder (Letterboxing) und stellt sicher, dass die App die gesamte Bildschirmfläche moderner iPhones verwendet.
* **Build-Fix #11 (Direkter ChatGPT-Chatmodus)**: Implementierung eines neuen Tabs mit einem vollwertigen ChatGPT-ähnlichen Chatverlauf. Ermöglicht direkte Interaktion mit der KI über eine eigenständige Server-Sitzung, ohne dass Screenshots oder Kurzbefehle nötig sind.
* **Build-Fix #12 (Tastatur-Verhalten optimiert)**: Hinzufügen einer Tipp-Geste zum Ausblenden der Tastatur und eines „Fertig“-Buttons in der Tastatur-Leiste, um zu verhindern, dass die Tastatur das Eingabefeld am unteren Rand verdeckt.
